<div class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center">
            @if ($todo->completed)
                <input type="checkbox" wire:click='toggle({{ $todo->id }})' checked class="mr-2">
            @else
                <input type="checkbox" wire:click='toggle({{ $todo->id }})' class="mr-2">
            @endif
            @if ($editTodoId === $todo->id)
                <div>
                    <input type="text" wire:model='editTodoName'
            ...
            @else
                <h3 class="text-lg text-semibold text-gray-800 ml-2">{{ $todo->name }}</h3>
            @endif
